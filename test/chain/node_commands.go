...
func (n *TestNode) Start(ctx context.Context) error {
	...
	n.C.T.Cleanup(func() {
		...
		if err != nil {
			n.C.T.Logf("failed to stop container %+v", err)
		}
	})
	return err
}
...
func (n *TestNode) Stop(ctx context.Context) error {
	...
	if err := n.C.Pool.Client.StopContainer(n.Container.ID, 10); err != nil {
		return err
	}
	return nil
}
...
func (n *TestNode) Run(ctx context.Context, cmd []string) (*dockertest.Resource, error) {
	n.C.T.Logf("{%s}[%s] -> '%s'", n.Name(), "", strings.Join(cmd, " "))
	return n.C.Pool.RunWithOptions(&dockertest.RunOptions{
		...
		Cmd:          cmd,
		Labels:       map[string]string{NODE_LABEL_KEY: n.C.T.Name()},
		NetworkID:    n.C.Network.ID,
...
func (n *TestNode) Execute(ctx context.Context, cmd []string) error {
	...
	code, err := n.C.Pool.Client.WaitContainerWithContext(resource.Container.ID, ctx)
	if err != nil {
		return err
	}
	if code != 0 {
